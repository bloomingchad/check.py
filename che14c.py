            ...
            async with session.get(url, timeout=15, allow_redirects=True) as response:
                ...
                if response.status not in range(200, 400):
                    if "ICY" in response.reason.upper():
                        return final_url, 200, "Success (ICY)", "audio/mpeg"
                    return final_url, response.status, "HTTP connection failed", None
                ...
                if any(content_type.startswith(valid_type) for valid_type in valid_content_types):
                    return final_url, response.status, "Success", content_type
                else:
                    return final_url, response.status, "Invalid Content-Type", content_type
...
async def process_station(station_name, url):
    ...
    try:
        final_url, status_code, reason, content_type = await check_http_connection_with_retry(url)
    except Exception as e:
        final_url, status_code, reason, content_type = None, 0, str(e), None
    ...
    # Use the final URL for mpv playback
    playback_url = final_url or url
    ...
    try:
        is_playing, failure_reason = await check_stream(player, playback_url)
        if is_playing:
