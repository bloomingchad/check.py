...
async def check_http_connection(url):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            # Use `raise_for_status=False` to avoid raising exceptions for non-2xx/3xx responses
            async with session.get(url, timeout=15, allow_redirects=True) as response:
                ...
                # Check status code
                if response.status not in range(200, 400) and response.status != 400:
                ...
                valid_content_types = [
                    ...
                    "application/ogg",  # Ogg Vorbis audio files
                    "audio/ogg",  # Ogg Vorbis or Opus audio files
...
@retry(stop=stop_after_attempt(3), wait=wait_fixed(2))
async def check_stream_with_retry(player, url):
    """Wrapper for check_stream with retry logic."""
    return await check_stream(player, url)

async def check_stream(player, url):
    playback_started = asyncio.Event()
    playback_failed = asyncio.Event()
    audio_detected = asyncio.Event()
    failure_reason = None  # Variable to store the reason for failure
    ...
    try:
        ...
        await asyncio.wait_for(playback_started.wait(), timeout=25)  # Increased timeout
...
async def process_station(station_name, url):
    ...
    if status_code == 0 and ("SSL" in reason or "handshake" in reason):
        ...
        with open("result.txt", "a") as f:
            f.write(result)
    elif status_code not in range(200, 400) and status_code != 400:  # Allow HTTP 400 to pass
    ...
    player = mpv.MPV(
        ...
        cache_secs=30,        # Set cache size to 30 seconds
        demuxer_max_back_bytes=10485760,  # 10 MB buffer for demuxer
        demuxer_max_bytes=10485760        # 10 MB buffer for demuxer
    )
    
    try:
        is_playing, failure_reason = await check_stream_with_retry(player, url)
        if is_playing:
            result = (
                f"✅ {station_name}\n"
                f"    - HTTP Status: {status_code}\n"
                f"    - Content-Type: {content_type}\n"
                f"    - Final URL: {player.playlist[0]['filename']}\n"
                f"    - Playback Verified: Yes\n"
            )
        else:
