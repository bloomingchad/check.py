...
async def check_stream(player, url):
    ...
    def on_property_change(property_name, value):
        if property_name == "idle-active" and not value:
            ...
            # Check if value is not None before accessing it
...
async def process_station(station_name, url):
    ...
    player = mpv.MPV(
        ao="null",           # Use null audio output
        audio_display=False, # Disable audio display
        vid=False,           # Disable video
        msg_level="all=error"  # Corrected value for msg-level
    )
