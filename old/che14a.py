...
async def adjust_concurrency_limit():
    """Periodically adjust the concurrency limit based on system load."""
    global semaphore
    while True:
        cpu_usage = psutil.cpu_percent(interval=1)
        memory_usage = psutil.virtual_memory().percent

        # Adjust concurrency limit based on CPU and memory usage
        if cpu_usage > 80 or memory_usage > 80:
            new_limit = max(MIN_CONCURRENCY_LIMIT, semaphore._value - 5)
        else:
            new_limit = min(MAX_CONCURRENCY_LIMIT, semaphore._value + 5)

        # Update the semaphore if the limit has changed
...
async def find_and_process_all_stations(directory):
    ...
    # Start the concurrency adjustment task
